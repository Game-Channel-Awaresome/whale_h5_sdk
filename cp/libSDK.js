...
function setLog(str, level) {
    if (SDK.debug == true) {
        if (level == 1) {
            console.log('%cSDK致命错误:' + str, 'color:red');
        } else if (level == 2) {
            console.log('%cSDK警告错误:' + str, 'color:gray');
        } else if (level == 0) {
            console.log('%cSDK运行日志:' + str, 'color:gray');
        } else {
            console.log('%cSDK运行日志:' + str, 'color:gray');
        }
    }
...
function checkOrderObject(orderInfo) {
    ...
    if (orderJson.productCode != SDK.productCode) {
...
//获取订单号
function getSDKOrderData(orderData, callback) {

    orderData.productCode = SDK.productCode;
...
var SDK = {
    ...
    pay: function(orderInfo, callback) {
        ...
        getSDKOrderData(orderData, function(getOrderData) {
    ...
    login: function(callback) {
        ...
        var messageObject = new Object();
        messageObject.func = 'Event_Login';
        window.parent.postMessage(JSON.stringify(messageObject), '*');
    },
    ...
    getUserInfo: function(callback) {
        ...
        var messageObject = new Object();
        messageObject.func = 'Event_GetUserInfo';
        window.parent.postMessage(JSON.stringify(messageObject), '*');
    },
    ...
    logout: function(callback) {
        ...
        var messageObject = new Object();
        messageObject.func = 'Event_Logout';
        window.parent.postMessage(JSON.stringify(messageObject), '*');
    },

    //上传角色信息
    uploadGameRoleInfo: function(roleInfo) {
        ...
        var messageObject = new Object();
        messageObject.func = 'Event_uploadRole';
    ...
    toggleModalMenu: function(status) {
        var messageObject = new Object();
        messageObject.func = 'Event_ToggleMenu';
...
window.addEventListener('message', function(e) {
    ...
    switch (funcName) {
        ...
        case 'Notify_Login':
            ...
            } else if (typeof(resObject) == 'object' && resObject.status == true) {
                setLog('登录校验成功:' + JSON.stringify(resObject), 0);
                CallbackLoginFun(resObject);

            } else {
                setLog('渠道返回的对象格式有误', 1);
                var errorObject = {
                	status:false,
                	data:{
                		uid:'',
        ...
        case 'Notify_GetUserInfo':
            CallbackGetUserInfoFun(messageObject.params);
            break;

        case 'Notify_payStatus':
